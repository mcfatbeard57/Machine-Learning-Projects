...
""" script to manually pass input files and parse MIO input xml file every hour and append the output to the daily output file"""
...
newest_filename = sys.argv[1] # newest_dir.split('/')[-1] # put / insted of \\
newest_dir = os.path.join(data_folder, sys.argv[1])
newest2_filename = sys.argv[2] # newest2_dir.split('/')[-1] # put / insted of \\
newest2_dir = os.path.join(data_folder, sys.argv[2])
...
# creating logs
#MIO_PM_Log_DDMMYYYY
logFilename = ('MIO_PM_Log_Manual_{}.log'.format(newest_filename[:-6]))
...
logger.info('This is the execution of the manual script with files \n')
logger.info('File1 is {} \n File2 is {}  '.format(newest_dir, newest2_dir))
...
try:
    ...
    logger.info('parsing of new file {} \n is succesful \n counts are following'.format(newest_dir))
...
try:
    ...
    logger.info('parsing of old file {} \n is succesful \n counts are following'.format(newest2_dir))
